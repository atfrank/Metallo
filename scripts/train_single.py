...
test_set_files = ["462d.txt"]


training_set_files = []
pdbID = test_set_files[0][0:4]
print(pdbID)
...
for training_set_rnas in training_set_files:
	try:
		...
		print("missing file in training set: " + training_set_rnas)
		continue
...
for test_set_rnas in test_set_files:
	try:
		...
		print("missing file in test set: " + test_set_rnas)
		continue
...
#report performance
print("f1_score precision recall test_acc train_acc\n")
print("{:.8f} ".format(f1_score(Y_test, predict))),
print("{:.8f} ".format(precision_score(Y_test, predict))),
print("{:.8f} ".format(recall_score(Y_test, predict))),
print("{:.8f} ".format(clf.score(X_test,Y_test))),
print("{:.8f}\n\n".format(clf.score(X,Y)))
print(np.array2string(confusion_matrix(Y_test, predict), separator=', '))
print("\n")
